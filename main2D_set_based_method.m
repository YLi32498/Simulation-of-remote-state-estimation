...
legend('$x_1$','$x_2$','Interpreter','latex','FontSize',20)
title('Plant trajectories','FontSize',20)
hold off
...
title('Dynamic Quantization Scheme using Zonotopes','FontSize',15)
...
%     [To;9_k{k},x{k}] = ode45(@(t,x) nlinearFode(t,x,A,H,n), [(k-1)*Tq k*Tq], [xk(:,k);Cd(:,k)]);
    ...
    xp{k} = x_obs(:,round((k-1).*Tq./step)+1:round(k.*Tq./step));           %store observer plant trajc
    x_p{k} = x_obs2(:,round((k-1).*Tq./step)+1:round(k.*Tq./step));         %store  plant trajc
...
plot(x',y','--','Color','r')
  ...
  ppp = scatter(Cd(1,k+1),Cd(2,k+1),40,[0 0 1],'filled');  
    ...
    Betad = beta_hat((k-1)*Tq)+gamma_hat;
%     Betad = gamma_hat
...
%         Gzz = [Gzz,Betad*Lam/N*eye(2),(Beta+Betad)*eye(2)];
...
legend([p(1) p(2) ppppp pp ppp pppppp],'local state estimate $\hat{x}(t)$','local state estimate at the transmission time $\hat{x}(t_k)$','quantization region $S_Q^k$','quantization subregions $S_q^k$','decoded state $P_d^k$','terminal reachable set $\mathcal{R}_T$','FontSize',15,'Interpreter','latex')
%%
...
% txt = 'L_q = [0.0571,0.0571]^T';
% text(0.25/2,2,txt,'FontSize',20)

%% testing for error convergance

L_q_ana = (eye(n)-LamHAT./N)^(-1)*[1;1]./N *(exp(Tq*norm(A,'inf'))-1)./norm(A,'inf')*(U + norm(K*C,'inf')*gamma_hat)



%% state reconstructor
sys_r = ss(A+K*C,-K*C,eye(n),0);
x_r_plot = [0;0];
x_plot = [0;0];
T_plot = [0];
for k = 1:length(T_total)-101
    x_r{k} = lsim(sys_r,Cd(:,k)*ones(1,length(T_k{k})),0:Tsim:Tq,Cd(:,k)); 
    x_r{k} = x_r{k}';
    T_plot = [T_plot T_k{k}];
