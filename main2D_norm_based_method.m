...
plot(x',y','--','Color','r')
  ...
  plotdecodedpacket = scatter(Pd(1,k+1),Pd(2,k+1),40,[0 0 1],'filled');  
    ...
    % quantization update law for norm-based method
    Cq(:,k+1) = Lam*Pd(:,k+1);      
    for i = 1:n
...
legend([p(1) p(2) plotz plotsubregion plotdecodedpacket],'local state estimate $\hat{x}(t)$','local state estimate at the transmission time $\hat{x}(t_k)$','quantization region $S_Q^k$','quantization subregions $S_q^k$','decoded state $P_d^k$','FontSize',15,'Interpreter','latex')
